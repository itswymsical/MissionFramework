using MissionFramework.Common.UI.Missions;
...
namespace MissionFramework.Core.Framework;
...
public abstract class Mission
{
    ...
    protected Mission(int id, string name, string description, List<List<(string, int)>> objectiveList,
        List<Item> rewards, bool isMainline, int providerNPC)
    {
        ID = id;
        Name = name;
    ...
    public void HandleObjectiveCompletion(int objectiveIndex)
    {
        ...
        {
            ModContent.GetInstance<MissionFramework>().Logger.Error($"Error in HandleObjectiveCompletion for mission {Name}: {ex.Message}");
        }
    }
