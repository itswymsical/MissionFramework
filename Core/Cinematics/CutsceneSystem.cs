...
/// <summary>
/// Handles the management and rendering of cutscenes.
/// </summary>
public class CutsceneSystem : ModSystem
{
    ...
    [
        ...
        "MissionFramework: Experience Meter",
        "MissionFramework: Pre-release info", // temporary
    ];
    ...
    private void DrawCutscene(On_Main.orig_DrawInterface orig, Main self, GameTime gameTime)
    {
        ...
        {
            ModContent.GetInstance<MissionFramework>().Logger.Error($"Error in DrawCutscene: {ex.Message}\nStack Trace: {ex.StackTrace}");
        }
    }
    ...
    public static void PlayCutscene<T>() where T : Cutscene, new()
    {
        try
        {
            if (CurrentCutscene != null)
            {
                ModContent.GetInstance<MissionFramework>().Logger.Warn("Attempting to start a new cutscene while one is already in progress. Ending the current cutscene.");
                CurrentCutscene.End();
            }
        ...
        {
            ModContent.GetInstance<MissionFramework>().Logger.Error("Error playing cutscene: " + ex.Message);
        }
    }
