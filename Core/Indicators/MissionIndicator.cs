using MissionFramework.Core.Dialogue;
using MissionFramework.Core.Framework;
using System.Linq;
using Terraria.GameContent;

namespace MissionFramework.Core.Indicators;
...
public class MissionIndicator : ScreenIndicator
{
    ...
    private void HandleClick()
    {
        try
        {
            ...
            ModContent.GetInstance<MissionFramework>().Logger.Info($"Player started {missionType} mission: {mission.Name}");
        }
        catch (Exception ex)
        {
            ModContent.GetInstance<MissionFramework>().Logger.Error($"Error starting mission {mission.Name}: {ex.Message}");
        }
    }
}